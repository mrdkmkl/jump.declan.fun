...
const COIN_VALUE     = 10;    // base coin value on platforms / shower (multipliers apply on top)
const BOUNCE_COIN_VALUE = 1;  // base coin value earned from bouncing on a platform (multipliers apply on top)
const MONEYBAG_VALUE = 100;  // rare high-value pickup found on some platforms
...
const MEGA_ITEMS = [
  {id:'mega_bronze',    icon:'🥉', name:'bronze vault',    desc:'permanent: shield + sticky boots on every run, forever',                          price:500,   type:'run', key:'megaBronze',    mega:true},
  {id:'mega_silver',    icon:'🥈', name:'silver vault',    desc:'permanent: double shield + mega boost tank on every run, forever',                price:750,   type:'run', key:'megaSilver',    mega:true},
  {id:'mega_gold',      icon:'🥇', name:'gold vault',      desc:'permanent: turbo + hyper boots + low gravity on every run, forever',              price:1000,  type:'run', key:'megaGold',      mega:true},
  {id:'mega_platinum',  icon:'💠', name:'platinum vault',  desc:'permanent: triple shield + free boosts + no dissolve on every run, forever',       price:1500,  type:'run', key:'megaPlatinum',  mega:true},
  {id:'mega_diamond',   icon:'💎', name:'diamond vault',   desc:'permanent: jetpack + quad jump + ultra start + wide body on every run, forever',   price:2000,  type:'run', key:'megaDiamond',   mega:true},
  {id:'mega_legendary', icon:'👑', name:'legendary vault', desc:'permanent: death save + score saver + lucky + wide platforms + boost regen, forever', price:10000, type:'run', key:'megaLegendary', mega:true},
];
...
const SKINS = [
  ...
  // ── low-end starter skins ──
  {id:'skin_pebble',   name:'pebble',    price:1000,      body:'#c9c9c9', accent:'rgba(70,70,70,.85)', special:null, coinMult:1.5},
  {id:'skin_coral',    name:'coral',     price:1250,     body:'#ff6b6b', accent:'rgba(140,20,20,.85)', special:null, coinMult:2},
  {id:'skin_lilac',    name:'lilac',     price:1600,     body:'#c9a8e0', accent:'rgba(90,55,110,.85)', special:null, coinMult:2.5},
  {id:'skin_seafoam',  name:'seafoam',   price:2000,     body:'#8fd9c4', accent:'rgba(20,80,60,.85)', special:null, coinMult:3.5},
  {id:'skin_peach',    name:'peach',     price:2500,     body:'#ffbfa0', accent:'rgba(140,70,30,.85)', special:null, coinMult:4.5},
  {id:'skin_sunbeam',  name:'sunbeam',   price:3200,    body:'#ffd93d', accent:'rgba(150,110,0,.85)', special:null, coinMult:5},
  {id:'skin_mint',     name:'mint',      price:4000,    body:'#6bcf9c', accent:'rgba(15,90,55,.85)', special:null, coinMult:7},
  {id:'skin_candy',    name:'candy stripe', price:5000, body:'#ff6b9d', colors:['#ff6b9d','#6bd6ff'], accent:'rgba(120,20,70,.85)', special:'stripes', coinMult:8},
  {id:'skin_sky',      name:'sky',       price:6300,    body:'#5aa9e6', accent:'rgba(10,60,110,.85)', special:null, coinMult:10},
  {id:'skin_sunset',   name:'sunset',    price:8000,    body:'#ff9a56', colors:['#ff9a56','#ff6b9d','#845ec2'], accent:'rgba(90,30,70,.9)', special:'gradient', coinMult:12},
  {id:'skin_grape',    name:'grape',     price:10000,   body:'#a66bd6', accent:'rgba(70,20,100,.85)', special:null, coinMult:14},
  {id:'skin_aurora',   name:'aurora',    price:12600,   body:'#00c9a7', colors:['#00c9a7','#4d8dff','#845ec2'], accent:'rgba(15,50,80,.9)', special:'gradient', coinMult:18},
  {id:'skin_galaxy',   name:'galaxy',    price:15900,   body:'#1b1240', colors:['#ff6bd6','#6bd6ff','#ffe66b','#a66bd6'], accent:'rgba(200,200,255,.85)', special:'galaxy', coinMult:22},
  {id:'skin_ember',    name:'ember',     price:25200,   body:'#ff7b3f', accent:'rgba(120,35,0,.85)', special:'glow', coinMult:27},
  {id:'skin_camo',     name:'tropical camo', price:39900, body:'#6bcf9c', colors:['#6bcf9c','#ffd93d','#ff6b6b','#4d8dff'], accent:'rgba(20,60,30,.9)', special:'camo', coinMult:32},
  {id:'skin_obsidian', name:'obsidian',  price:63200,  body:'#2b2b3a', accent:'rgba(180,180,220,.85)', special:'glow', coinMult:36},
  // ── metal series (mid-high end) ──
  {id:'skin_copper',   name:'copper',    price:20000,   body:'#c07a4a', accent:'rgba(90,45,15,.9)', special:'metal', coinMult:25},
  {id:'skin_silver',   name:'silver',    price:31700,   body:'#c7ccd1', accent:'rgba(70,75,80,.9)', special:'metal', coinMult:29},
  {id:'skin_titanium', name:'titanium',  price:50200,   body:'#8a92a6', accent:'rgba(40,45,60,.9)', special:'metal', coinMult:34},
  {id:'skin_platinum', name:'platinum',  price:79600,  body:'#e2e6ea', accent:'rgba(90,95,100,.9)', special:'metal', coinMult:40},
  {id:'skin_sunny',    name:'sunny',     price:126200,  body:'#ffe14d', accent:'rgba(150,110,0,.9)', special:'sunny', coinMult:45},
  {id:'skin_gold',     name:'gold',      price:100200,  body:'#ffcc33', accent:'rgba(110,70,0,.9)', special:'shine', coinMult:43},
  {id:'skin_holo',     name:'holo',      price:158800,  body:'holo',    accent:'rgba(60,20,90,.85)', special:'holo', coinMult:48},
  {id:'skin_diamond',  name:'diamond',   price:200000, body:'#bdf3ff', accent:'rgba(20,80,100,.9)', special:'shine', coinMult:95},
];
...
let savedCoins = 0;
let highScores = {easy:0, normal:0, hard:0, chaos:0};
let owned = {};
let purchaseCounts = {};
let activeToggles = {};
let runPowerups = {};
...
let curseDeclineFee = 10; // cost to skip a cursed power-up — starts at 10, +20 each decline (resets every run)
...
function coinGain(base=COIN_VALUE){
  const mult = cheatState.coinMultiplierX10 ? 10 :
  ...
  return Math.max(1, Math.round(base * mult * skinMult * rebirthMult));
}
...
function startGame(){
  ...
  ALL_SEASONAL_ITEMS.forEach(item => {
    ...
    switch(item.key){
      ...
      case 'wreathShield':  runPowerups.wreathShield=true; runPowerups.shieldCount=(runPowerups.shieldCount||0)+2; break;
      ...
      case 'snowball':      break; // handled in initPlayer (ultra boost)
      ...
      case 'irishJig':      runPowerups.turbo=true; break;
      ...
      case 'springLamb':    runPowerups.turbo=true; break;
...
function update(dt){
  ...
  const sec=dt*(1/60);
  if(tempSpring>0)      tempSpring      =Math.max(0,tempSpring      -sec);
  ...
  if(tempNoCoin>0)      tempNoCoin      =Math.max(0,tempNoCoin      -sec);
  ...
  const _rawDiff = cheatState.freezeDiff ? 1.0 : Math.min(2.0, 1+Math.floor(score/500)*DIFF_SCALE_PER_1000M*500);
  ...
  if(keys.boost&&!keys.boostHeld&&!keys.shiftHeld){
    if(boosts>0||cheatState.infBoosts||freeBoostsLeft>0){
      ...
      player.vy=_bf*(springActive?1.15:hyperActive?1.3:1)*(runPowerups.doubleBoost?1.7:1);
      if(freeBoostsLeft>0){
  ...
  if(keys.boost&&keys.shift&&!keys.ultraHeld){
    if(ultraBoosts>0){
      player.vy=ULTRA_BOOST_FORCE*(springActive?1.15:1);
  ...
  // score
  const _baseDiv = Math.max(1, Math.round(cheatState.scoreDivisor / cheatState.scoreMultiplier));
  ...
  for(let i=platforms.length-1;i>=0;i--){
    ...
    if(player.vy>=0&&!dashingThrough&&!(p.type==='dissolving'&&ghostActive&&p.touched)){
      ...
      if(effPlayerX+effPlayerW-2>p.x&&effPlayerX+2<p.x+p.w&&ppy<=p.y&&py>=p.y){
        ...
        const bounceCoinGain= (tempNoCoin>0) ? 0 : coinGain(BOUNCE_COIN_VALUE);
    ...
    if(p.hasBag&&!p.bagCollected){
      ...
      if(bdist<bagMagnetR){
        if(bdist<24){
          p.bagCollected=true;
          const skinMult = (getEquippedSkin().coinMult) || 1;
          const bagGain = Math.max(1, Math.round(MONEYBAG_VALUE * skinMult));
...
function getDayNightT(){
  if(cheatState.forceNight) return 1;
...
function buildShopCard(item, discountMap){
  ...
  card.className='shop-card'+(isOwned?' owned':'')+((!canAfford&&!isOwned)?' cannot-afford':'')+(deal?` on-sale-${deal.tier}`:'')+(item.mega?' mega-card':'');
...
function rebirthCost(){
  return Math.round(100000 * Math.pow(2, rebirthCount));
}
...
function openStatsOverlay(){
  ...
  ['easy','normal','hard','chaos'].forEach(m=>{
...
window.addEventListener('keydown',e=>{
  ...
  if(e.key===settings.pauseKey||e.key==='Escape'||e.key==='p')togglePause();
});
...
document.getElementById('ch-reset-hs').addEventListener('click', e=>{
  highScores = {easy:0, normal:0, hard:0, chaos:0};
